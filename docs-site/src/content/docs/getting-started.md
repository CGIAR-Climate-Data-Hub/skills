...
Fastest way in: install every skill in this repo with one command via the
...
This works with Claude Code, Antigravity, Codex, Cursor, and 70+ other agents — pick the ones you
want when the CLI prompts you. Re-run it any time to pick up updates, or add `--skill <name>` to
install just one (e.g. `--skill gcf-pipeline`).
